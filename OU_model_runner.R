...
# Match data index with tree
data_key <- data |>
  group_by(glottocode) |>
  summarise(lang_id = unique(lang_id)) |>
  arrange(lang_id)

tree_order <- match(data_key$glottocode, tree$tip.label)

## Create phylo distance matrix
dist_mat <- cophenetic.phylo(tree) # pairwise distance matrix using branch lengths
dist_mat <- dist_mat / max(dist_mat) # scale tree
dist_mat2 <- dist_mat[tree_order, tree_order] # put in right order
...
## Compile stan code
PGLMM_BM <- cmdstan_model(here::here("models/OU_with_age-sex.stan"))
...
# Save fit
fit$save_object(file = "results/fit_OU_with_age-sex.RDS")


PGLMM_BMA <- cmdstan_model(here::here("models/OU_with_age-sex.stan"))
...
# Save fit
fit2$save_object(file = "results/fit_OU_with_age-sex.RDS")
