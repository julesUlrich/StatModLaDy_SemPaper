data {
  ...
  vector[N_speaker] sex_s; vector[N_speaker] age_s;
...
parameters {
  real p_0;
  real a_speaker, b_speaker_sex, b_speaker_age;
  real<lower=0> sigma_p_speaker, sigma_p_lang, sigma_p_phoneme, sigma_p_conversation;
  vector[N_speaker] p_speaker_z; vector[N_lang] p_lang_z;
...
transformed parameters {
  ...
  vector[N_speaker] p_speaker_v = a_speaker + b_speaker_sex*sex_s + b_speaker_age*age_s + sigma_p_speaker*p_speaker_z;
...
model {
  p_0 ~ normal(0,1);
  a_speaker ~ normal(0,1); b_speaker_sex ~ normal(0,1); b_speaker_age ~ normal(0,1);
  sigma_p_speaker ~ normal(0,1); sigma_p_lang ~ normal(0,1);
  sigma_p_phoneme ~ normal(0,1); sigma_p_conversation ~ normal(0,1);
  p_speaker_z ~ std_normal(); p_lang_z ~ std_normal();
  p_phoneme_z ~ std_normal(); p_conversation_z ~ std_normal();
  sigmasq_0 ~ normal(0,1); sigma_sigmasq_phoneme ~ normal(0,1); sigmasq_phoneme_z ~ std_normal();
